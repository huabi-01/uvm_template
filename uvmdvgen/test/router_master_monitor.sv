`ifndef router_master_monitor
`define router_master_monitor
    class router_master_monitor extends uvm_monitor#(router_transaction);
        `uvm_component_utils(router_master_monitor)
        
        uvm_analysis_port#(router_transaction) mst_mon_ap;
         vif;
        
        function new(string name = "router_master_monitor",uvm_component parent);
